...
{
	/// <summary>
	/// Descripción breve de TiposObservacionesProveedores.
	/// </summary>
	public class TiposObservacionesProveedores: Agrupamientos, ITiposObservaciones
	{
		#region Constructores
		public TiposObservacionesProveedores()
		{
		}
		#endregion
		...
		public override void Commit() 
		{
			foreach(object obs in _tiposObservacionesNuevas)
			{
				...
				if (on.Modo == "NEW")
				{
					tpu_ProveedoresObservacionesTiposDataset.tpu_ProveedoresObservacionesTiposRow row = _data.tpu_ProveedoresObservacionesTipos.Newtpu_ProveedoresObservacionesTiposRow();
					row.Descripcion = on.DescripcionTipo;
					row.FechaCreacion = DateTime.Now;
					row.Activo = true;
					row.IdConexionCreacion = Security.IdConexion;
					row.IdProveedorObservacionTipo = on.KeyTipo;
					if(on.KeyTipoPadre != "RAIZ")
						row.IdProveedorObservacionTipoPadre = on.KeyTipoPadre;
					row.IdReservado = 0;
					row.IdEmpresa = Security.IdEmpresa;			
					row.IdSucursal = Security.IdSucursal;
					_data.tpu_ProveedoresObservacionesTipos.Addtpu_ProveedoresObservacionesTiposRow(row);
					...
					{
						idVariable1 = "Proveedores.Observaciones.Add.SubTipos";
						idVariable2 = "Proveedores.Observaciones.Ver.SubTipos";	
					...
					{
						idVariable1 = "Proveedores.Observaciones.Add.Tipos";
						idVariable2 = "Proveedores.Observaciones.Ver.Tipos";
						foreach(sy_PerfilesDataset.sy_PerfilesRow rowP in _tablePerfiles.Rows)
						{
							this.agregarVariable(idVariable1, rowP.IdPerfil);
							this.agregarVariable(idVariable2, rowP.IdPerfil);
				...
				{
					DataRow rowT = mz.erp.businessrules.tpu_ProveedoresObservacionesTipos.GetByPk(on.KeyTipo);
					rowT["Descripcion"] = on.DescripcionTipo;
					rowT["Activo"] = on.Activo;
					if(on.KeyTipoPadre == "RAIZ")
						rowT["IdProveedorObservacionTipoPadre"] = System.DBNull.Value;
					_data.tpu_ProveedoresObservacionesTipos.ImportRow(rowT);
				}
			}
		...
		private mz.erp.commontypes.SentenciasReplicacion GenerateReplication()
		{
			...
			replication.NombreTabla = "dataaccess.tpu_ProveedoresObservacionesTipos";			
			return replication;
		}
		...
		public override void Flush(string IdTransaction)
		{
			...
			dataaccess.tpu_ProveedoresObservacionesTipos.Update(_data.tpu_ProveedoresObservacionesTipos, trx);
			bool modificaPerfiles = false;
			if( this._processParent.Process.ProcessName.Equals("ProcesoAbmTiposObservacionesProveedores")) 
			{//Si es la configuracion rapida agrego en los nuevos tipos de observaciones para q las vean todos los perfiles existentes en el sistema
				//Si es personalizada (ProcesoAbmTiposObservacionesProveedoresPersonalizado) se le mustra la interfaz para configurar estas variables.
				if(_modificaPerfilesSubTipos)
				{
					int i = 0;
					foreach(tpu_ProveedoresObservacionesTiposDataset.tpu_ProveedoresObservacionesTiposRow row in _data.tpu_ProveedoresObservacionesTipos.Rows)
					{
						TipoObservacionNueva on = (TipoObservacionNueva)_tiposObservacionesNuevas[i];
						if( on.Modo.Equals("NEW") && !row.IsIdProveedorObservacionTipoPadreNull())
						{
							string IdProveedorObservacionTipo = (string)row["IdProveedorObservacionTipo"];
							foreach(sy_PerfilesDataset.sy_PerfilesRow rowP in _tablePerfiles.Rows)
							{
								long IdPerf = rowP.IdPerfil;

								int index1 = this.getIndexOf("Proveedores.Observaciones.Add.SubTipos", IdPerf);
								if(existeVariable("Proveedores.Observaciones.Add.SubTipos", IdPerf))
								{
								...
								_dataPeriles.sy_VariablesPerfiles.Rows[index1]["Valor"] = _dataPeriles.sy_VariablesPerfiles.Rows[index1]["Valor"] + IdProveedorObservacionTipo;
								
								int index2 = this.getIndexOf("Proveedores.Observaciones.Ver.SubTipos", IdPerf);
								if(existeVariable("Proveedores.Observaciones.Ver.SubTipos", IdPerf))
								{
								...
								_dataPeriles.sy_VariablesPerfiles.Rows[index2]["Valor"] = _dataPeriles.sy_VariablesPerfiles.Rows[index2]["Valor"] + IdProveedorObservacionTipo;
							}
							//_variablesValores.Add(on.DescripcionTipo, IdProveedorObservacionTipo);
				...
				if(_modificaPerfilesTipos)
				{
					...
					foreach(tpu_ProveedoresObservacionesTiposDataset.tpu_ProveedoresObservacionesTiposRow row in _data.tpu_ProveedoresObservacionesTipos.Rows)
					{
						...
						if(on.Modo.Equals("NEW") && row.IsIdProveedorObservacionTipoPadreNull())
						{
							...
							{
								long IdPerf = rowP.IdPerfil;
								int index1 = this.getIndexOf("Proveedores.Observaciones.Add.Tipos", IdPerf);
								if(existeVariable("Proveedores.Observaciones.Add.Tipos", IdPerf))
								{
								...
								int index2 = this.getIndexOf("Proveedores.Observaciones.Ver.Tipos", IdPerf);
								if(existeVariable("Proveedores.Observaciones.Ver.Tipos", IdPerf))
								{
		...
		public ArrayList getContext()
		{
			...
			context.Add("Proveedores.Observaciones.Add.SubTipos");
			context.Add("Proveedores.Observaciones.Ver.SubTipos");
			context.Add("Proveedores.Observaciones.Add.Tipos");
			context.Add("Proveedores.Observaciones.Ver.Tipos");
			return context;
		}
		...
		private tpu_ProveedoresObservacionesTiposDataset _data = new tpu_ProveedoresObservacionesTiposDataset();
		...
		public override Node getChildsNode(string key)
		{
			...
			{
				table = mz.erp.businessrules.tpu_ProveedoresObservacionesTipos.GetListTipos().Tables[0];
				header = new Node(key, "Tipos de Observaciones");
			}
			else
			{
				table = mz.erp.businessrules.tpu_ProveedoresObservacionesTipos.GetListSubTipos(key).tpu_ProveedoresObservacionesTipos;
			...
			foreach (System.Data.DataRow rowH in table.Rows) 
			{
				...
				keyH = rowH["IdProveedorObservacionTipo"].ToString();
		...
		private void ValidarDatos()
		{
			...
			foreach(object obs in _tiposObservacionesNuevas)
			{
				...
				{
					_errores.Add( new ItemDatasetError( "Proveedores Observaciones Tipos","Descripcion","El campo descripción no puede estar vacío") );
				}
				...
				{
					tpu_ProveedoresObservacionesTiposDataset DataSetTipos;
					if(on.KeyTipoPadre == "RAIZ")
						DataSetTipos = mz.erp.businessrules.tpu_ProveedoresObservacionesTipos.GetList("", false);
					else
						DataSetTipos = mz.erp.businessrules.tpu_ProveedoresObservacionesTipos.GetList(on.KeyTipoPadre, false);

					if (DataSetTipos.Tables[0].Rows.Count>0 )
					{
						foreach (tpu_ProveedoresObservacionesTiposDataset.tpu_ProveedoresObservacionesTiposRow _row in  DataSetTipos.Tables[0].Rows)
							if ((string)_row["Descripcion"]== descripcion)
							{
								_errores.Add( new ItemDatasetError( "Proveedores Observaciones Tipos","Descripcion","Ya existe otro tipo con ese nombre") );	
							}
		...
		private int getIndexOf(string idVariable, long idPerfil)
		{
			...
			while(index < 0 && _dataPeriles.sy_VariablesPerfiles.Rows.Count > i)
			{
				...
				if(idV.Equals(idVariable) && idP == idPerfil)
					index = i;
				else 
					i++;
			}	
