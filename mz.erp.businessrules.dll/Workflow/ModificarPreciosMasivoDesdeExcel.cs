                    ...
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                        if (!(Convert.ToString(row["Codigo"])).Equals(string.Empty))
                        {
                            ...
                            else if (PorCodigoProveedor)
                            {
                                ...
                                string IdProveedor = tpu_Proveedores.GetPKByCodigo(this.CodigoProveedor);
                                DataTable IdProductos = tsh_Productos.GetListIdProductoByCodigoProveedorIdProveedor(CodigoIngresado, IdProveedor);
                                if (IdProductos == null)
                                {
                                    IdProducto = null;
                                }
                                else
                                {
                                    ...
                                    string IdProveedorAux = tpu_Proveedores.GetPKByCodigo(this.CodigoProveedor);
                                    
                                    foreach (DataRow rowProd in IdProductos.Rows)
                                    {
                                        IdProducto = Convert.ToString(rowProd["IdProducto"]);

                                        tpu_ProveedoresProductosDataset.tpu_ProveedoresProductosRow rowPP = tpu_ProveedoresProductos.GetByPk(IdProveedorAux, IdProducto);
                                        if (rowPP == null) //No existe el producto en tpu_ProveedoresProductos
                                        {
                                            Estado = "NEW_LP";
                                        }
                            ...
                            if (!PorCodigoProveedor)
                            {
                                ...
                                if (!aux.Contains(CodigoIngresado))
                                {
                                    ...
                                    if (validaRepetidos && !_productosRepetidos.Contains(CodigoIngresado))
