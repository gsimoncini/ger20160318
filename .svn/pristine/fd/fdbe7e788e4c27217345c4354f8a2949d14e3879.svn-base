...
Public Class UnhandledExceptionManager
    ...
    Private Shared _blnLogToDB As Boolean
    ...
    Private Shared _blnLogToDBOK As Boolean
    ...
    Private Shared _strEmailTo As String
    Private Shared _strEmailPort As String
    Private Shared _strEmailServer As String
...
#Region "Properties"
    ...
    Public Shared Property LogToDB() As Boolean
        Get
            Return _blnLogToDB
        End Get
        Set(ByVal Value As Boolean)
...
#End Region
    ...
    Private Shared Sub LoadConfigSettings()
        SendEmail = mz.erp.businessrules.Variables.GetValueBool("Debug.ManejadorDeExcepciones.HabilitaEnvioMail")
        ...
        LogToDB = GetConfigBoolean("LogToDB", False)
        LogToDB = mz.erp.businessrules.Variables.GetValueBool("Debug.ManejadorDeExcepciones.HabilitaEnvioDB")
        'DisplayDialog = GetConfigBoolean("DisplayDialog", True)
        DisplayDialog = mz.erp.businessrules.Variables.GetValueBool("Debug.ManejadorDeExcepciones.HabilitaEnvioPantalla")
        IgnoreDebugErrors = GetConfigBoolean("IgnoreDebug", True)
        KillAppOnException = GetConfigBoolean("KillAppOnException", True)

        _strEmailTo = mz.erp.businessrules.Variables.GetValueString("Debug.ManejadorDeExcepciones.Mail.To")
        _strEmailPort = mz.erp.businessrules.Variables.GetValueString("Debug.Mail.Port")
        _strEmailServer = mz.erp.businessrules.Variables.GetValueString("Debug.Mail.Server")
    ...
    Public Shared Sub [AddHandler](ByVal blnConsoleApp As Boolean, ByVal sendMail As Boolean, ByVal emailTo As String, ByVal emailPort As String, ByVal emailServer As String, ByVal logToDB As Boolean, ByVal displayDialog As Boolean)
        '-- attempt to load optional settings from .config file
        LoadConfigSettings()
        SendEmail = sendMail
        _strEmailTo = emailTo
        _strEmailPort = emailPort
        _strEmailServer = emailServer
        logToDB = logToDB
    ...
    '--
    Public Shared Sub GenericExceptionHandlerToDB(ByVal objException As Exception)
        ...
        Catch ex As Exception
            _strException = "Error '" & ex.Message & "' mientras se generaba el string de excepción"
            _strExceptionType = ""
        End Try
        ...
        Try
            ...
            If _blnLogToDB Then ExceptionToDB(objException)
        Catch ex As Exception
            Debug.WriteLine(ex)
    ...
    Private Shared Function FormatExceptionForUser(ByVal blnConsoleApp As Boolean) As String
        ...
        With objStringBuilder
            If Not blnConsoleApp Then
                ...
                .Append("El equipo de desarrollo fué automáticamente notificado de éste problema. ")
                '.Append("If you need immediate assistance, contact (contact).")
                .Append("Si necesita asistencia inmediata contacte a ().")
            End If
            .Append(Environment.NewLine)
            .Append(Environment.NewLine)
            '.Append("The following information about the error was automatically captured: ")
            .Append("La siguiente información acerca del error fué automáticamente capturada: ")
            .Append(Environment.NewLine)
            .Append(Environment.NewLine)
            If _blnLogToScreenshot Then
                ...
                If _blnLogToScreenshotOK Then
                    ...
                    .Append("se ha capturado la pantalla en:")
                    .Append(Environment.NewLine)
                    .Append("   ")
                    .Append(_strScreenshotFullPath)
                Else
                    '.Append("a screenshot could NOT be taken of the desktop.")
                    .Append("no se ha podido realizar una captura de pantalla del escritorio.")
                End If
                .Append(Environment.NewLine)
            End If
            If _blnLogToEventLog Then
                ...
                If _blnLogToEventLogOK Then
                    ...
                    .Append("un evento fué escrito en el log de aplicaciones")
                Else
                    '.Append("an event could NOT be written to the application log")
                    .Append("no se ha podido escribir en el log de aplicaciones")
                End If
                .Append(Environment.NewLine)
            End If
            If _blnLogToFile Then
                ...
                If _blnLogToFileOK Then
                    ...
                    .Append("detalles fueron escritos en el log de texto en:")
                Else
                    '.Append("details could NOT be written to the text log at:")
                    .Append("no de han escrito detalles en el log de texto en:")
                End If
                .Append(Environment.NewLine)
                .Append("   ")
            ...
            If _blnLogToEmail Then
                ...
                .Append("se ha intentado enviar un mail a: ")
                .Append(Environment.NewLine)
                .Append("   ")
                .Append(_strEmailTo)
                .Append(Environment.NewLine)
            ...
            .Append("Información detallada a continuación:")
            .Append(Environment.NewLine)
            .Append(Environment.NewLine)
    ...
    Private Shared Sub ExceptionToUI()
        ...
        Const _strWhatHappened As String = "Se ha encontrado un error inesperado en (Mz.Erp)."
        ...
        Const _strWhatUserCanDo As String = "Reinicie (Mz.Erp) e intente repetir la última acción. Intente métodos alternativos para realizar la misma operación."

        If UnhandledExceptionManager.KillAppOnException Then
            '_strHowUserAffected = "When you click OK, (app) will close."
            _strHowUserAffected = "Cuando haga clic en OK, (Mz.Erp) se cerrará."
        Else
            '_strHowUserAffected = "The action you requested was not performed."
            _strHowUserAffected = "La acción que ha intentado realizar no se ha podido finalizar."
        End If
    ...
    Private Shared Sub ExceptionToConsole()
        Debug.WriteLine("This application encountered an unexpected problem.")
        Debug.WriteLine(FormatExceptionForUser(True))
        Debug.WriteLine("The application must now terminate. Press ENTER to continue...")
        Console.ReadLine()
    End Sub
    '--
    ...
    Private Shared Sub ThreadHandler()
        Dim objMail As New mz.erp.security.dll.Mail.SMTPClient
        Dim objMailMessage As New mz.erp.security.dll.Mail.SMTPMailMessage
        With objMailMessage
            .To = _strEmailTo
            '.Subject = "Unhandled Exception notification - " & _strExceptionType
            .Subject = "Notificación de Excepción no controlada - " & _strExceptionType
            .Body = _strException
        ...
        End With
        Try
            objMail.Server() = _strEmailServer
            objMail.Port() = Convert.ToInt32(_strEmailPort)
    ...
    '--
    '-- send an exception via DB
    '--
    Private Shared Sub ExceptionToDB(ByVal objException As Exception)
        Dim maquina As String = Environment.MachineName '-- SISTEMAS-07
        Dim usuario As String = UserIdentity() '-- SISTEMAS-07\oscar
        Dim version As String = ParentAssembly.GetName().Version().ToString '-- 1.4.4.1909
        Dim excSource As String = objException.Source '-- mz.erp.ui
        Dim excType As String = objException.GetType.FullName() '-- System.Exception
        Dim excMensaje As String = objException.Message() '-- Se inició una excepción de tipo System.Exception.
        Dim excTargetSite As String = objException.TargetSite.Name '-- ProcessTool
        Dim excStackTrace As String = objException.StackTrace
        Dim excInnerException As String = Convert.ToString(objException.InnerException)
        Dim excHelpLink As String = objException.HelpLink

        Dim exception As String = _strException '-- (la excepción completa. Conteniendo también los datos anteriores)

        Dim appDomain As String '--mz.erp.ui.exe
        Dim codebase As String '--file:///C:/Fuentes/MZ.ERP/mz.erp.ui.exe/bin/Debug/mz.erp.ui.exe

        Try
            appDomain = System.AppDomain.CurrentDomain.FriendlyName()
        Catch e As Exception
            appDomain = excMensaje
        End Try

        Try
            codebase = ParentAssembly.CodeBase()
        Catch e As Exception
            codebase = excMensaje
        End Try

        mz.erp.businessrules.sy_Excepciones.AddExcepcion(maquina, usuario, version, excSource, excType, excMensaje, excTargetSite, excStackTrace, excInnerException, excHelpLink, exception, appDomain, codebase)
    ...
    Friend Shared Function SysInfoToString(Optional ByVal blnIncludeStackTrace As Boolean = False) As String
        ...
        With objStringBuilder
            ...
            .Append("Nombre completo del Assembly:    ")
            Try
            ...
            .Append("Fecha de generación del Assembly:   ")
            Try
