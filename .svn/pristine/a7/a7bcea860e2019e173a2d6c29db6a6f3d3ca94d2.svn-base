...
namespace mz.erp.ui.controls
{
	public class mzComboSearchEditor : System.Windows.Forms.UserControl
	{
		...
		private string GetToolTipString()
		{
			...
			if (_captionHint != String.Empty) 
			{
				if (_selectedRow != null)
				{
					ArrayList captionHintList = Util.Parse(_captionHint, ",");
					ArrayList HintList = Util.Parse(_hint, ",");
					for( int i = 0; i < HintList.Count; i++ )
					{					
						string name = Convert.ToString(HintList[i]).Trim();
						string valor = Convert.ToString(_selectedRow[name]);
						string hintvalor = Convert.ToString(captionHintList[i]);
						s += hintvalor + ": " + valor + "\r\n";
					}							
				}
			}			
		...
		private void labelDescription_Click(object sender, System.EventArgs e)
		{
			if (editorID.Text != string.Empty) 
			{
				MessageBox.Show(GetToolTipString());
			}			
		}
		...
		public bool ReadOnly
		{
			set
			{
				editorID.ReadOnly = value;
				if (value)
					buttonElipsis.Click -= new EventHandler(buttonElipsis_Click);
				
			}
			get
			{
				return editorID.ReadOnly;
			}
		}
