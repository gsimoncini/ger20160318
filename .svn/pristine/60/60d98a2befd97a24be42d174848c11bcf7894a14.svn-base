...
{
	/// <summary>
	/// Descripción breve de SeleccionReparacionesPendientes.
	/// </summary>
	public class SeleccionReparacionesPendientes : ITask
	{
		public SeleccionReparacionesPendientes()
		{
			//
		...
		public string IdEstado
		{
			...
			set
			{
				if (value == String.Empty)
					_idEstado = _idEstadosPermitidos;
				else
					_idEstado = value;
		...
		public void Init()
		{
			...
			//this._codigoCuenta = Variables.GetValueString("Momentos." +_primaryTaskName+ ".Cuenta.Default");
			...
			this._idEstadosPermitidos = Variables.GetValueString("Momentos." + _primaryTaskName + "." + processName+".EstadosOrdenDeReparacion");
			this._idEstado = Variables.GetValueString("Momentos." + _primaryTaskName + "." + processName+".EstadoOrdenDeReparacion.Default");
			...
			{
				string idEstado = Convert.ToString(row["Codigo"]);
				if(estados.Contains(idEstado))
					_tableEstadosFiltrados.ImportRow(row);
			}
			*/
		...
		public DataTable RefreshData()
		{
			...
			long idPrimaryTask = Convert.ToInt64(Workflow.GetProcessPrimaryTask(this.GetProcessManagerParent().Process.IdProcess).Tables[0].Rows[0]["IdTarea"]);
			if(Workflow.GetPreviousTasksOfProcess(idPrimaryTask,this.GetProcessManagerParent().Process.IdProcess).Tables[0].Rows.Count > 0)
			{
				System.Data.DataRow row = Workflow.GetPreviousTasksOfProcess(idPrimaryTask,this.GetProcessManagerParent().Process.IdProcess).Tables[0].Rows[0];
				esEquipoNuevo = Convert.ToString(row["Descripcion"]).Equals("ArmarEquipoNuevo");
			}
			return mz.erp.businessrules.reportes.ReportsFactory.ReparacionesPendientesWF(this._idOrdenReparacion,this._idCuenta,this._fechaDesde, this._fechaHasta,this._idEstado, esEquipoNuevo);
		}
		#endregion
