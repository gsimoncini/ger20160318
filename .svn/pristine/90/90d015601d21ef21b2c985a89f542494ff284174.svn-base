	...
	public class LayoutBuilder
	{
		...
		public static string GetRow( int position, string fieldName, string caption, int width, Types.Aligment aligment,Types.FormatDisplay format, string literalstring )
		{
			switch (format)
			{
				...
				{
					return
						string.Format(@"<Column{0} ID=""{1}""><Caption>{2}</Caption><DataMember>{1}</DataMember><Key>{1}</Key><Width>{3}</Width><Position>{0}</Position><TextAlignment>{4}</TextAlignment><FormatString>dd/MM/yyyy</FormatString></Column{0}>",
						new object[] {position, fieldName, caption, width, aligment.ToString()}); 
					break;
				}
