...
Public Class HandledExceptionManager
    ...
    Private Shared _blnEmailError As Boolean = False
    ...
    Private Shared _blnLogToEmail As Boolean = True
    ...
    Private Shared Function ExceptionToMore(ByVal objException As System.Exception) As String
        ...
        With sb
            If _blnEmailError Then
                ...
                .Append("Información acerca de éste problema ha sido automáticamente enviada por mail a ")
            ...
            .Append("A continuación se detalla información técnica: " + Environment.NewLine)
            .Append("---" + Environment.NewLine)
    ...
    Private Shared Sub ThreadHandler()
        Dim smtp As New mz.erp.security.dll.Mail.SMTPClient
        Dim mail As New mz.erp.security.dll.Mail.SMTPMailMessage
        With mail
            ...
            If _blnHaveException Then
                ...
                .Subject = "Notificación de manejo de Excepción - " & _strExceptionType

            Else
                '.Subject = "HandledExceptionManager notification"
                .Subject = "HandledExceptionManager notificación"
            End If
            .Body = _strEmailBody
        End With
        '-- try to send email, but we don't care if it succeeds (for now)
        Try
            smtp.Server() = AppSettings.GetString("UnhandledExceptionManager/Server")
            smtp.Port() = AppSettings.GetInteger("UnhandledExceptionManager/Puerto")
            smtp.SendMail(mail)
    ...
    '--
    Public Shared Sub SendNotificationEmail(ByVal strWhatHappened As String, _
        ByVal strHowUserAffected As String, _
        ByVal strWhatUserCanDo As String, _
